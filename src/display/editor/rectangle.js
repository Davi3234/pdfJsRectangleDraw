...
import { RectangleAnnotationElement } from "../annotation_layer.js";
import { noContextMenu } from "../display_utils.js";
import { opacityToHex } from "./tools.js";

/**
 * Basic draw editor in order to generate an Rectangle annotation.
 */
class RectangleEditor extends AnnotationEditor {
  ...
  static _editorType = AnnotationEditorType.RECTANGLE;
  ...
  get propertiesToUpdate() {
    return [
      ...
      [
        AnnotationEditorParamsType.RECTANGLE_OPACITY,
        Math.round(100 * (this.opacity ?? RectangleEditor._defaultOpacity)),
      ],
      [
  ...
  /**
   * Seta os atributos do desenho/fill
   */
  #setStroke() {
   ...
   * @param {number} bolLast -> Verifica se é a última execução para desenhar o final do desenho
   */
  #draw(x, y, bolLast) {
  ...
  //Desenha em tempo de excução um esboço
  #drawPoints() {
  ...
  #redraw() {
    ...
    this.#updateTransform();
    ctx.clearRect(this.startX, this.startY, this.widthRect, this.heightRect);
    this.#setStroke();
    ctx.fillRect(this.startX, this.startY, this.widthRect, this.heightRect);
  }
   ...
   */
    ...
    // Since the rectangle editor covers all of the page and we want to be able
  ...
  #createCanvas() {
    ...
    this.canvas.setAttribute("data-l10n-id", "pdfjs-rectangle-canvas");
  ...
  render() {
    ...
    this.div.setAttribute("data-l10n-id", "pdfjs-rectangle");
  ...
  setDimensions(width, height) {
    ...
    this.widthRect = Math.ceil(width);
    this.heightRect = Math.ceil(height);
  ...
  #serializePaths(s, tx, ty, rect) {
    ...
    for (const bezier of this.paths) {
      ...
      paths.push({
        bezier: RectangleEditor.#toPDFCoordinates(buffer, rect, this.rotation),
        points: RectangleEditor.#toPDFCoordinates(points, rect, this.rotation),
      });
    }
  ...
  static deserialize(data, parent, uiManager) {
    if (data instanceof RectangleAnnotationElement) {
      return null;
    }
    ...
    for (let { bezier } of paths) {
      bezier = RectangleEditor.#fromPDFCoordinates(bezier, rect, rotation);
      const path = [];
